...
struct Tensor {
    ...
    Tensor(unsigned int const shape_[N]) {
        ...
        for (unsigned int i = 0; i < N; ++i) {
            shape[i] = shape_[i];  // Copy shape
            size *= shape[i];       // Compute total elements
        }
        data = new T[size];
...
private:
    unsigned int data_index(unsigned int const indices[N]) const {
        ...
        unsigned int stride = 1;  // Stride for each dimension (starts at 1)

        for (int i = N - 1; i >= 0; --i) {  // Process dimensions in reverse order
            ASSERT(indices[i] < shape[i], "Invalid index");  // Bounds check
            index += indices[i] * stride;
            stride *= shape[i];  // Update stride for next dimension
        }
